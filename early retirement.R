...
df <- all %>%
  ...
  dplyr::filter(cost_subtype == "Early retirement" |
                  cost_subtype == "early retirement") %>%
...
##################################################################################
####Last step is to impute sd for 833:
#lets calculate cv from study 458, and then estimate sd for each cases and control groups.
# cv = sd/mean
df$cv <- df$sd_converted/df$mean_converted
cv458case <-df %>%
  filter(population == "T2DM", s_id == 458) %>%
  summarise(cv)

cv458ctrl <-df %>%
  filter(population == "noDM", s_id == 458) %>%
  summarise(cv)
#impute the cv for missing ones
df[df$s_id == 833 & df$population == "T2DM", "cv"] <- cv458case
df[df$s_id == 833 & df$population == "noDM", "cv"] <- cv458ctrl
...
forest(m_cost,common = F, 
       main = "Pooled Ratio of Mean Early Retirement Costs (T2DM vs Controls)",
       xlab = "Ratio of Means",
       backtransf = TRUE)
###############################################################################
