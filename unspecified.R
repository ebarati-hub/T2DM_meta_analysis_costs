...
df <- all %>%
  dplyr::filter(complication_cat == "stroke") %>%
  dplyr::filter(cost_type == "Productivity") %>%
  dplyr::filter(cost_subtype == "Productivity loss") %>%
...
df$ci_upper <- as.numeric(as.character(df$ci_upper))
df$ci_lower <- as.numeric(as.character(df$ci_lower))
...
forest(m_overall_logn,
       ...
       xlab = "Geometric mean of unspecified productivity loss in T2DM population (2024 CAD, back-transformed)")
