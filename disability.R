...
df <- all %>%
  ...
  dplyr::filter(cost_subtype == "Disability benefits" |
                  cost_subtype == "LT Disability" |
                  cost_subtype == "ST Disability") %>%
...
# Approach:
#   1) Sum the mean costs (ST + LT)
#   2) Calculate the combined SD using the standard formula for independent estimates
#   3) Use the smaller sample size (N) as a conservative choice
# Note:
#   Participants are not identical across ST and LT groups, but both come 
#   from the same underlying cohort (N ≈ 202,000).
# Action: create a combined (LT+ST) row for study ID 113
r <- 0.8 
pair_cases113 <- subset(df,
                        s_id == 113 &
                          population == "T2DM" &
                          cost_subtype %in% c("LT Disability","ST Disability"))

mean_total_cases113 <- sum(pair_cases113$mean, na.rm = TRUE)
se_total_cases113   <- with(pair_cases113, sqrt(se[1]^2 + se[2]^2 + 2*r*se[1]*se[2]))
n_cases113 <- min(pair_cases113$n)
...
##################################################################################
#pooled mean for study 599. Action: sum the adjusted means and divide by 10
# sd is NR
# N is the same
...
####Last step is to impute sd for 599:
#lets calculate cv from study 113, and then estimate sd for each cases and control groups.
# cv = sd/mean
df$cv <- df$sd_converted/df$mean_converted
cv113case <-df %>%
  filter(population == "T2DM", s_id == 113) %>%
  summarise(cv)

cv113ctrl <-df %>%
  filter(population == "noDM", s_id == 113) %>%
  summarise(cv)
#impute the cv for missing ones
df[df$s_id == 599 & df$population == "T2DM", "cv"] <- cv113case
df[df$s_id == 599 & df$population == "noDM", "cv"] <- cv113ctrl
